...
import { formatPropertyValue } from "@/lib/leads";
import {
  getHeyReachImport,
  ...
  importRunCanEnrich,
  importRunIsActive,
  withHeyReachLastEnrichment,
  type CampaignLastImport,
  type HeyReachCampaign,
  type HeyReachImportRun,
} from "@/lib/heyreach";
import {
...
type HeyReachCampaignDetailDrawerProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  campaign: HeyReachCampaign | null;
  importPending: boolean;
  importError: string;
  onImport: (campaign: HeyReachCampaign) => void;
};

export function HeyReachCampaignDetailDrawer({
  open,
  onOpenChange,
  campaign,
  importPending,
  importError,
  onImport,
}: HeyReachCampaignDetailDrawerProps) {
  return (
    <Drawer open={open} onOpenChange={onOpenChange} swipeDirection="right">
      <DrawerContent className="sm:[--drawer-content-width:36rem]">
        <DrawerHeader className="relative pr-12">
          ...
          <DrawerDescription>
            {campaign
              ? "Import LinkedIn conversations, then enrich phones for those leads."
              : "Campaign imports"}
          </DrawerDescription>
          <DrawerClose
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
          {campaign ? (
            <div className="flex flex-col gap-6">
              ...
              <SpeedToLeadSettings source="heyreach" campaign={campaign} />

              <HeyReachImportPanel
                open={open}
                campaign={campaign}
                lastImport={heyreachCampaignLastImport(campaign)}
...
function HeyReachImportPanel({
  ...
  const importBusy = importPending || heyreachCampaignImportIsActive(campaign);

  const detailQuery = useQuery({
    queryKey: heyreachImportKeys.detail(runId ?? ""),
    queryFn: ({ signal }) => getHeyReachImport(runId ?? "", signal),
  ...
  const run: HeyReachImportRun | CampaignLastImport | null = detailQuery.data ?? lastImport;
    ...
    <section className="flex flex-col gap-4 rounded-xl border border-border/70 bg-muted/20 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-col gap-1">
          <h3 className="text-sm font-medium text-foreground">LinkedIn conversations</h3>
          <p className="text-xs text-muted-foreground">
            {campaign.imported_lead_count} imported lead
            {campaign.imported_lead_count === 1 ? "" : "s"}
          </p>
        </div>
        <Button
          type="button"
      ...
      {run ? (
          ...
          <Section label="Phone enrichment">
            {enrichment ? (
              ...
              <p className="text-sm text-muted-foreground">
                Enrich phones for leads from this import.
              </p>
            )}
        ...
        <p className="text-sm text-muted-foreground">
          Import LinkedIn conversations for this campaign to capture leads and enrich phones.
        </p>
      )}
    </section>
  );
